    ...
    <section class="px-5 py-8 sm:px-8 lg:px-12">
      <div class="mx-auto max-w-[1500px]">
        ...
        <div class="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p class="text-sm font-semibold uppercase tracking-[0.2em] text-brand-red">Shopping Cart</p>
            <h1 class="mt-2 text-3xl font-bold sm:text-4xl">Review your order</h1>
          </div>
          <NuxtLink to="/" class="text-sm font-medium underline underline-offset-4">Continue shopping</NuxtLink>
        </div>
        ...
        <div class="cart-shell">
          <section class="cart-list-card">
            ...
            <div>
              <div v-for="item in cartItems" :key="item.name" class="cart-line-item">
                <div class="flex min-w-0 flex-1 gap-4">
                  ...
                  <div class="min-w-0">
                    <p class="text-sm font-medium text-[#777]">{{ item.brand }}</p>
                ...
                <div class="flex h-9 w-28 shrink-0 overflow-hidden rounded-full border border-[#d8d8d8] bg-white">
                  ...
                  <span class="grid flex-1 place-items-center text-sm font-medium">{{ item.qty }}</span>
                  <button class="grid w-9 place-items-center text-base transition hover:bg-[#fafafa]" @click="increase(item.name)">+</button>
                </div>

                <p class="w-36 shrink-0 text-left text-lg font-semibold md:text-right">{{ formatRupiah(item.price * item.qty) }}</p>
            ...
            <div v-if="!cartItems.length" class="p-12 text-center">
              ...
              <NuxtLink to="/" class="mt-6 inline-flex bg-brand-red px-6 py-3 text-sm font-semibold uppercase tracking-wide text-white">Start shopping</NuxtLink>
            </div>
          </section>
            ...
            <div class="rounded-xl border border-[#eeeeee] bg-white p-6 shadow-xl">
              ...
              <div class="mt-6 space-y-4 text-sm">
                ...
                <div class="flex justify-between">
                  ...
                  <span class="font-medium">{{ deliveryFee ? formatRupiah(deliveryFee) : 'Free' }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-[#666]">Packaging</span>
                  <span class="font-medium">{{ formatRupiah(packagingFee) }}</span>
                </div>
              </div>

              <div class="mt-6 rounded-lg bg-[#fafafa] p-4">
                <label class="text-xs font-semibold uppercase tracking-wide text-[#777]">Promo code</label>
                <div class="mt-2 flex gap-2">
                  ...
                  <button class="h-11 bg-[#333] px-4 text-sm font-semibold text-white transition hover:bg-brand-red">Apply</button>
                </div>
              </div>
              ...
              <div class="mt-6 border-t border-[#eeeeee] pt-5">
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-2xl font-bold">{{ formatRupiah(total) }}</span>
                </div>
                <p class="mt-2 text-xs leading-5 text-[#777]">Taxes and final delivery fees are calculated at checkout.</p>
              </div>

              <button class="mt-6 h-12 w-full bg-brand-red text-sm font-semibold uppercase tracking-wide text-white transition hover:bg-[#5b21b6]">
                Checkout
              </button>
            </div>

            <div class="mt-5 rounded-xl border border-[#eeeeee] bg-white p-5 shadow-sm">
              <h3 class="font-semibold">Delivery perks</h3>
